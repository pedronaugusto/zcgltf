#!/usr/bin/env bash
#
# zcgltf — the CI matrix, run locally.
#
...
[ $LIST -eq 1 ] ||
  printf '%szcgltf local CI%s  %s%s%s\n' "$BOLD" "$OFF" "$DIM" "$($ZIG version)" "$OFF"
...
# consumer's link — so zcgltf's own Debug run asks for it explicitly. This is
# the run that would catch undefined behaviour reachable from our tests.
...
if [ $QUICK -eq 0 ]; then
  ...
  # Consuming zcgltf as a dependency is a different code path from building
...
# Windows runner. There is no shared-library configuration to build: upstream
# declares no export macro, so zcgltf is static-only (see README's Scope).
fi
