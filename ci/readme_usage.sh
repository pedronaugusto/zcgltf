#!/usr/bin/env bash
#
# zcgltf — README.md's Usage snippet, extracted from examples/usage.zig.
#
...
imports = [
    line for line in text.splitlines() if line.startswith('const zcgltf = @import(')
]
if len(imports) != 1:
    sys.exit("%s: expected exactly one `const zcgltf = @import(...)` line" % source)
