#!/usr/bin/env bash
#
# zcgltf — recompute every number the documentation claims.
#
...
# The upstream C surface and its Zig mirror. Declarations sit above each
# header's implementation guard; the first line of a declaration starts at
# column 0 and names a `cgltf_` function. src/abi_check.zig's reverse sweep
# fails the build if any header export lacks an extern, so the two counts can
# only disagree through a bug in this script.
count_decls() {
  awk -v GUARD="$2" '$0 ~ GUARD { exit }
    /^[A-Za-z_][A-Za-z0-9_* ]*cgltf_[a-z0-9_]+\(/ { count++ }
    END { print count + 0 }' "$1"
}
header_fns=$(( $(count_decls libs/cgltf/cgltf.h '^#ifdef CGLTF_IMPLEMENTATION') \
             + $(count_decls libs/cgltf/cgltf_write.h '^#ifdef CGLTF_WRITE_IMPLEMENTATION') ))
emit upstream_entry_points "$header_fns" \
  'upstream C entry points (declared in the vendored `cgltf.h` + `cgltf_write.h`)'

externs=$(grep -hc '^pub extern fn cgltf_' src/c/*.zig | sum)
...
test_count=$(printf '%s\n' "$build_log" |
  sed -n 's/.*run test zcgltf-tests \([0-9][0-9]*\) pass.*/\1/p' | head -1)
