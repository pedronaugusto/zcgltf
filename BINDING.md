...
is the single source. `build.zig` reads it into the options module,
`version()` re-exports it, and a test compares the two so the mirror cannot
rot. README's version cell is generated by `ci/measurements.sh` from the
same field.
